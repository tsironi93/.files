...
-- /*                                                +#+#+#+#+#+   +#+           */
-- /*   Created: %s by %s%s#+#    #+#             */
-- /*   Updated: 2025/05/27 16:25:56 by itsiros          ###   ########.fr       */
-- /*                                                                            */
-- /* ************************************************************************** */
-- ]],
--     filename,
...
-- lua/config/42header.lua

local M = {}

function M.Insert42Header()
  local filename = vim.fn.expand("%:t")
  local user = os.getenv("USER") or "user"
  local git_email = vim.fn.system("git config user.email"):gsub("[\n\r]", "")
  local email = git_email ~= "" and git_email or user .. "@student.42.fr"

  local filename_padding = 51 - #filename
  local email_padding = 40 - (#user + #email)
  local created_user_padding = 18 - #user
  local updated_user_padding = 17 - #user
...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: %s by %s%s#+#    #+#             */
/*   Updated: 2025/05/27 16:30:15 by itsiros          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
]],
    filename,
